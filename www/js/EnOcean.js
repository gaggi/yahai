...
function EnOcean_send(name) {														// sending EnOcean status changes 
	if(name.getAttribute("data-web-type") == 'dimmer') {
		...
		if(name.id == name.getAttribute("data-actor")+'up') {
			...
			console.log('SENDING: trigger nForTimer '+name.getAttribute("data-send-actor")+' up 0.1 released');							// some logging
		} else if(name.id == name.getAttribute("data-actor")+'stop') {
			console.log('SENDING: trigger nForTimer '+name.getAttribute("data-send-actor")+' stop 0.1 released');						// some logging
			...
			console.log('SENDING: trigger nForTimer '+name.getAttribute("data-send-actor")+' down 0.1 released');						// some logging
		} 
	} 
};
